/*
	...
	Revision history:
		...
		1.01 (07/22/2026) GLSL ES 3.00 transpiler backend (CSPV_Options.emit_glsl300),
		                  replacing SPIRV-Cross for CF's GLES3/WebGL2 path. Emits
		                  straight from the checked AST -- no SPIR-V decompilation.
		                  Inserts the explicit conversions ES requires (ES has no
		...
		1.03 (07/23/2026) MSL transpiler backend (CSPV_Options.emit_msl) following
		                  SDL_GPU's Metal binding contract. MSL's C++ nature keeps
		                  it close to GLSL (column-major matrices, native
		                  operators); mutable globals dissolve into a shader
		...
		1.07 (08/03/2026) sampler2DArrayShadow (vec4 uv + layer + reference, the
		                  hardware-PCF cascaded-shadow-map sampler) across all four
...
typedef struct CSPV_Result
{
	...
	// On success, when CSPV_Options.emit_msl was set - the shader transpiled to
	// MSL (a ckit string) following SDL_GPU's Metal binding contract; hand the
...
struct cspv_expr
{
	cspv_expr_kind kind;
	int line;
	// Stamped during SPIR-V codegen (which doubles as the type checker), for the
	// transpiler backends: `rtype` is the expression's own resolved type; `want`
	// is the type an enclosing context implicitly converted it to (decl inits,
	// call arguments, returns, ternary arms). Text targets without 450-style
	// implicit conversions (GLSL ES) use these to insert explicit conversions.
	struct cspv_type* rtype;
	struct cspv_type* want;
	union
	{
		double fval;
		uint64_t ival;
...
// Recorded top-level declarations, in source order. SPIR-V emission is fused
// into parsing; the transpiler backends (GLSL ES) replay the module from these
// instead. All interior arrays are arena-allocated, so cleanup is just the
// containing dynamic array.
typedef enum cspv_decl_kind
{
	CSPV_D_STRUCT,  // struct definition (type holds the fields).
	CSPV_D_GLOBAL,  // global (Private) variable, possibly const, with optional init.
	CSPV_D_INOUT,   // stage in/out variable.
	CSPV_D_BLOCK,   // uniform or buffer block.
	CSPV_D_OPAQUE,  // sampler2D/usampler2D/image2D uniform.
	CSPV_D_SHARED,  // compute shared variable.
	CSPV_D_FUNC,    // function definition (body AST is arena-retained).
} cspv_decl_kind;

typedef struct cspv_decl
{
	cspv_decl_kind kind;
	int line;
	cspv_type* type;           // Struct type / var type / opaque type / function return type.
	const char* name;
	// CSPV_D_INOUT:
	bool is_input;
	bool flat;                 // Includes the implicit flat on integer varyings.
	int location;
	// CSPV_D_GLOBAL:
	bool is_const;
	...
	int num_members;
	cspv_type** member_types;  // Arena arrays.
	const char** member_names;
	// CSPV_D_FUNC:
	int num_params;
	cspv_param* params;        // Arena array.
	cspv_stmt* body;
} cspv_decl;
...
typedef struct cspv_ctx
{
	...
	cspv_type* t_sampler2darrayshadow;
	...
	// OpTypeImage id behind each sampler type, keyed by cspv_type* (texelFetch and
	// textureSize's OpImage result type must match the sampled image's image type).
	CK_MAP(uint32_t) sampler_image_tids;
...
static uint32_t cspv_type_id(cspv_ctx* ctx, cspv_type* type)
{
	...
	if (type->kind == CSPV_T_VEC) {
		...
		case CSPV_SDIM_2D_ARRAY_SHADOW: arrayed = 1; depth = 1; break;
		}
		...
		map_set(ctx->sampler_image_tids, (uint64_t)(uintptr_t)type, img);
	}
...
static const char* cspv_type_name(cspv_type* t)
{
	...
	case CSPV_T_SAMPLER2D:
		...
		case CSPV_SDIM_2D_ARRAY_SHADOW: return "sampler2DArrayShadow";
...
static uint32_t cspv_gen_rvalue_as(cspv_ctx* ctx, cspv_expr* e, cspv_type* want)
{
	...
	e->want = want; // Recorded for the transpiler backends (see cspv_expr).
...
static cspv_value cspv_gen_intrin(cspv_ctx* ctx, cspv_expr* e, cspv_intrin* in)
{
	...
	if (in->kind == CSPV_INTRIN_TEXTURE || in->kind == CSPV_INTRIN_TEXTURE_OFFSET ||
		...
		bool shadow = sdim == CSPV_SDIM_2D_SHADOW || sdim == CSPV_SDIM_CUBE_SHADOW ||
		              sdim == CSPV_SDIM_2D_ARRAY_SHADOW;
		// texelFetch/textureOffset cover the dims GLSL defines them for (textureSize
		// works on every dim); the sampled-image ops below handle every dim.
		if (in->kind == CSPV_INTRIN_TEXEL_FETCH &&
		    sdim != CSPV_SDIM_2D && sdim != CSPV_SDIM_2D_ARRAY && sdim != CSPV_SDIM_3D) {
			cspv_errorf(ctx, e->line, "texelFetch requires a sampler2D, sampler2DArray, or sampler3D (GLSL defines no texelFetch for %s)", cspv_type_name(st));
		}
		if (in->kind == CSPV_INTRIN_TEXTURE_OFFSET && sdim != CSPV_SDIM_2D && sdim != CSPV_SDIM_2D_ARRAY) {
			cspv_errorf(ctx, e->line, "textureOffset requires a sampler2D or sampler2DArray");
		}
		...
		// reference), and vec4 for cube shadow (xyz direction + w reference) and 2D array
		// shadow (xy + z layer + w reference); shadow references split below for SPIR-V's
		// separate Dref operand.
		cspv_type* coord_t = sdim == CSPV_SDIM_2D ? ctx->t_vec[0] :
		                     (sdim == CSPV_SDIM_CUBE_SHADOW || sdim == CSPV_SDIM_2D_ARRAY_SHADOW) ? ctx->t_vec[2] : ctx->t_vec[1];
		...
		if (in->kind == CSPV_INTRIN_TEXTURE) {
			...
			if (shadow) {
				...
				// vec4(dir, ref)) / texture(sampler2DArrayShadow, vec4(uv, layer, ref))
				// yield a float visibility factor. Split the reference off the trailing
				// coordinate component.
				bool wide = sdim == CSPV_SDIM_CUBE_SHADOW || sdim == CSPV_SDIM_2D_ARRAY_SHADOW;
				uint32_t xy = cspv_new_id(ctx);
				uint32_t sw[8] = { cspv_type_id(ctx, wide ? ctx->t_vec[1] : ctx->t_vec[0]), xy, coord, coord, 0, 1, 2 };
				cspv_emit(&ctx->body, CSpvOpVectorShuffle, sw, wide ? 7 : 6);
				uint32_t dref = cspv_new_id(ctx);
				cspv_emit4(&ctx->body, CSpvOpCompositeExtract, cspv_type_id(ctx, ctx->t_float), dref, coord, wide ? 3u : 2u);
		...
		if (in->kind == CSPV_INTRIN_TEXTURE_OFFSET) {
			// The offset must be a constant expression (SPIR-V ConstOffset operand). It
			// stays ivec2 for array textures too: ConstOffset excludes the layer component.
			uint32_t coord = cspv_gen_rvalue_as(ctx, e->u.call.args[1], coord_t);
		...
		if (in->kind == CSPV_INTRIN_TEXTURE_LOD) {
			if (sdim == CSPV_SDIM_CUBE_SHADOW || sdim == CSPV_SDIM_2D_ARRAY_SHADOW) {
				cspv_errorf(ctx, e->line, "textureLod is not supported with %s (GLSL has no such overload)", cspv_type_name(st));
			}
			uint32_t coord = cspv_gen_rvalue_as(ctx, e->u.call.args[1], coord_t);
		...
		// texelFetch/textureSize operate on the image, not the sampled image. The
		// OpImage result type is the sampler's OpTypeImage, recorded per sampler
		// type when cspv_type_id emitted it (loading the sampler emitted it).
		cspv_type_id(ctx, st); // Ensures the image tid exists in the map below.
		uint32_t img_tid = *map_get_ptr(ctx->sampler_image_tids, (uint64_t)(uintptr_t)st);
		uint32_t image = cspv_new_id(ctx);
		cspv_emit3(&ctx->body, CSpvOpImage, img_tid, image, sampler);

		// Array and 3D dims carry three integer coordinate/size components.
		bool three = sdim == CSPV_SDIM_2D_ARRAY || sdim == CSPV_SDIM_3D || sdim == CSPV_SDIM_2D_ARRAY_SHADOW;

		if (in->kind == CSPV_INTRIN_TEXEL_FETCH) {
			uint32_t coord = cspv_gen_rvalue_as(ctx, e->u.call.args[1], three ? ctx->t_ivec[1] : ctx->t_ivec[0]);
		...
		// textureSize: ivec2 for 2D/cube dims (a cube face's extent), ivec3 for
		// array (width, height, layers) and 3D (width, height, depth).
		ctx->needs_image_query = true;
		cspv_type* size_t_ = three ? ctx->t_ivec[1] : ctx->t_ivec[0];
		uint32_t lod = cspv_gen_rvalue_as(ctx, e->u.call.args[1], ctx->t_int);
		uint32_t result = cspv_new_id(ctx);
		cspv_emit4(&ctx->body, CSpvOpImageQuerySizeLod, cspv_type_id(ctx, size_t_), result, image, lod);
		return cspv_rvalue(size_t_, result);
	}
	...
	for (int i = 0; i < argc; i++) {
		...
		e->u.call.args[i]->want = ft; // For the transpiler backends.
	...
	if (fn) {
		...
		for (int i = 0; i < argc; i++) {
			...
			if (qual) {
				...
				e->u.call.args[i]->want = chosen->params[i]; // For the transpiler backends.
...
static cspv_value cspv_gen_expr_impl(cspv_ctx* ctx, cspv_expr* e);

// Wrapper so every generated expression records its resolved type on the AST
// node, for the transpiler backends. Codegen is the type checker, so by the
// time any transpiler runs, every reachable expression has been stamped.
static cspv_value cspv_gen_expr(cspv_ctx* ctx, cspv_expr* e)
{
	cspv_value v = cspv_gen_expr_impl(ctx, e);
	e->rtype = v.type;
	return v;
}
...
// Append a zeroed top-level declaration record for the transpiler backends.
static cspv_decl* cspv_record_decl(cspv_ctx* ctx, cspv_decl_kind kind, int line)
{
	cspv_decl d;
	memset(&d, 0, sizeof(d));
	d.kind = kind;
...
static void cspv_gen_function(cspv_ctx* ctx, cspv_type* ret, const char* name, CK_DYNA cspv_param* params, cspv_stmt* body, int line)
{
	...
	{
		int n = (int)asize(params);
		cspv_decl* d = cspv_record_decl(ctx, CSPV_D_FUNC, line);
		d->type = ret;
		d->name = name;
		d->num_params = n;
		if (n) {
			d->params = (cspv_param*)cspv_arena_alloc(&ctx->arena, sizeof(cspv_param) * n);
			memcpy(d->params, params, sizeof(cspv_param) * n);
		}
		d->body = body;
	}
}
...
static void cspv_gen_global_inout(cspv_ctx* ctx, cspv_layout* layout, bool is_input, bool flat, cspv_type* type, const char* name, int line)
{
	...
	cspv_decl* d = cspv_record_decl(ctx, CSPV_D_INOUT, line);
	d->type = type;
	d->name = name;
	d->is_input = is_input;
	d->flat = flat || integer_varying;
	d->location = layout->location;
}
...
static void cspv_gen_uniform_block(cspv_ctx* ctx, cspv_layout* layout, const char* block_name, bool is_buffer, bool readonly, int line)
{
...

	{
		...
		d->num_members = n;
		d->member_types = (cspv_type**)cspv_arena_alloc(&ctx->arena, sizeof(cspv_type*) * n);
		d->member_names = (const char**)cspv_arena_alloc(&ctx->arena, sizeof(const char*) * n);
		memcpy(d->member_types, member_types, sizeof(cspv_type*) * n);
		memcpy(d->member_names, member_names, sizeof(const char*) * n);
	}
...
static void cspv_gen_struct_decl(cspv_ctx* ctx)
{
	...
	cspv_decl* d = cspv_record_decl(ctx, CSPV_D_STRUCT, line);
	d->type = t;
	d->name = name;
}
...
static void cspv_gen_global_var(cspv_ctx* ctx, bool is_const, cspv_type* type, const char* name, int line)
{
	...
	cspv_decl* d = cspv_record_decl(ctx, CSPV_D_GLOBAL, line);
	d->type = type;
	d->name = name;
	d->is_const = is_const;
	d->init = init_expr;
...
static void cspv_gen_top_level(cspv_ctx* ctx)
{
	while (ctx->tok.kind != CSPV_TOK_EOF) {
		...
		if (cspv_accept_ident(ctx, g_cspv_kw.kw_shared)) {
			...
			cspv_decl* d = cspv_record_decl(ctx, CSPV_D_SHARED, line);
			d->type = type;
			d->name = name;
			continue;
		}
...
//--------------------------------------------------------------------------------------------------
// GLSL ES 3.00 transpiler. Replays the module from the recorded declarations
// (cspv_decl) and the retained, type-stamped statement/expression ASTs. Runs
// after SPIR-V generation, so every reachable expression carries its resolved
// type (rtype) and any implicit conversion codegen applied to it (want).
//
// ES-specific transforms:
//   - ES 3.00 has no 450-style implicit int -> float conversions: explicit
//     conversions are inserted wherever codegen recorded one (`want`), and
//     binary/ternary operands are re-balanced with typed literals or
//     constructor casts.
//   - Varyings drop layout(location) (illegal in ES 3.00) and are renamed to
//     canonical location-keyed names so separately-compiled stages link by name.
//   - gl_VertexIndex/gl_InstanceIndex become gl_VertexID/gl_InstanceID.
//   - The vertex entry point is renamed and wrapped by a main() that applies
//     CF's clip-space conventions -- depth [0,w] -> [-w,w] and a y-flip --
//     matching SPIRV-Cross's FIXUP_DEPTH_CONVENTION + FLIP_VERTEX_Y.
...
typedef struct cspv_tp
{
	cspv_ctx* ctx;
	int indent;
	bool hlsl;                        // Dialect: HLSL SM 5.1.
	bool msl;                         // Dialect: MSL 2.x. (Neither set: GLSL ES 3.00.)
...
{
	// MSL shares HLSL's value-type spellings (float4, float4x4, ...).
	return (g->hlsl || g->msl) ? cspv_hlsl_type_name(t) : cspv_type_name(t);
}
...
static const char* cspv_tp_op_str(int op)
{
	...
	case CSPV_P_SHR: return ">>";
	case CSPV_P_LE: return "<=";
	case CSPV_P_GE: return ">=";
	case CSPV_P_EQ: return "==";
	case CSPV_P_NE: return "!=";
	case CSPV_P_AND: return "&&";
	case CSPV_P_OR: return "||";
	case CSPV_P_ADD_ASSIGN: return "+=";
	case CSPV_P_SUB_ASSIGN: return "-=";
	case CSPV_P_MUL_ASSIGN: return "*=";
	case CSPV_P_DIV_ASSIGN: return "/=";
...
static int cspv_tp_prec(int op)
{
	...
	case '+': case '-': return 12;
	case CSPV_P_SHL: case CSPV_P_SHR: return 11;
	case '<': case '>': case CSPV_P_LE: case CSPV_P_GE: return 10;
	case CSPV_P_EQ: case CSPV_P_NE: return 9;
	case '&': return 8;
	case '^': return 7;
	case '|': return 6;
...
static void cspv_msl_call(cspv_tp* g, cspv_expr* e);
	...
	case CSPV_E_BINARY: {
		...
		if (cspv_tp_is_assign_op(op)) {
			...
			if (cspv_tp_numeric(lt) && cspv_tp_numeric(rt) &&
				...
				if (lk != rk) {
					cspv_type* elem = cspv_promo_rank(lk) >= cspv_promo_rank(rk) ? cspv_elem_type(lt) : cspv_elem_type(rt);
					ltarget = cspv_vec_type(g->ctx, elem, cspv_num_components(lt));
...
{
	if ((g->hlsl || g->msl) && init->kind == CSPV_E_CALL && init->u.call.array_size != -1) {
...
static void cspv_tp_stmt(cspv_tp* g, cspv_stmt* s)
{
	...
	case CSPV_S_SWITCH: {
		// ES requires case labels to match the selector's signedness. (HLSL and
		// MSL have implicit conversions, so plain int labels are always fine.)
		cspv_type* st = cspv_tp_rtype(g, s->u.switch_s.sel);
		bool sel_uint = !g->hlsl && !g->msl && st && cspv_elem_type(st)->kind == CSPV_T_UINT;
		cspv_tp_indent(g);
	...
	case CSPV_S_DISCARD:
		cspv_tp_indent(g);
		sappend(g->ctx->tp_out, g->msl ? "discard_fragment();\n" : "discard;\n");
		break;
...
static void cspv_emit_glsl300(cspv_ctx* ctx)
{
	...
	for (int i = 0; i < (int)asize(ctx->decls); i++) {
		...
		bool varying = (ctx->stage == CSPV_STAGE_VERTEX && !d->is_input) ||
		               (ctx->stage == CSPV_STAGE_FRAGMENT && d->is_input);
		if (!varying) continue;
	...
	for (int i = 0; i < (int)asize(ctx->decls); i++) {
		...
		case CSPV_D_SHARED:
			break; // Unreachable: compute was rejected above.
		}
	}
...
static const char* cspv_hlsl_sampler_tex(const cspv_type* t)
{
	...
	case CSPV_SDIM_2D_ARRAY_SHADOW: return "Texture2DArray<float4>";
...
{
	return t->cols == CSPV_SDIM_2D_SHADOW || t->cols == CSPV_SDIM_CUBE_SHADOW || t->cols == CSPV_SDIM_2D_ARRAY_SHADOW
...
static const char* cspv_msl_sampler_tex(const cspv_type* t)
{
	...
	case CSPV_SDIM_2D_ARRAY_SHADOW: return "depth2d_array<float>";
...
static void cspv_hlsl_call(cspv_tp* g, cspv_expr* e)
{
	...
	if (argc >= 1 && args[0]->kind == CSPV_E_REF && args[0]->rtype && args[0]->rtype->kind == CSPV_T_SAMPLER2D) {
		...
		if (shadow && (!strcmp(name, "texture") || !strcmp(name, "textureLod"))) {
			...
			sappend(*out, hwide ? ").xyz, (" : ").xy, (");
			cspv_tp_expr(g, args[1], 2);
			sappend(*out, hwide ? ").w)" : ").z)");
			return;
		}
		...
		if (!strcmp(name, "texelFetch")) {
			// Load's coordinate carries the mip as its last component: int3(xy, mip)
			// for Texture2D, int4(xy + layer or xyz, mip) for Texture2DArray/Texture3D.
			bool hload4 = hdim == CSPV_SDIM_2D_ARRAY || hdim == CSPV_SDIM_3D;
			sfmt_append(*out, "%s_tex.Load(%s(", s, hload4 ? "int4" : "int3");
			cspv_tp_expr(g, args[1], 2);
			sappend(*out, ", ");
			cspv_tp_expr(g, args[2], 2);
			sappend(*out, "))");
...
//--------------------------------------------------------------------------------------------------
// MSL transpiler. Shares the statement/expression printer with the GLSL/HLSL
// backends (cspv_tp with g->msl set). MSL is C++-based, which dissolves the
// two problems the other dialects fought: matrices are column-major with
// native operator* (no mul()/transpose games), and the mutable-global problem
// is solved structurally -- all of the shader's globals become members of one
// `struct cf_shader` and every user function becomes a method on it, so
// member access needs no threading of state through call graphs. The entry
// point (named main0; MSL reserves `main`) aggregate-initializes the struct
// with the stage's resources and bridges stage IO.
//
// Resource indices follow SDL_GPU's MSL contract (matching what
// SDL_shadercross computed via spirv-cross remapping):
//   graphics:  [[texture/sampler(n)]] n = binding; uniform [[buffer(binding)]];
//              storage [[buffer(uniform_count + binding - texture_count)]]
//   compute:   RO textures n = binding; RW textures n = ro_texture_count + binding;
//              uniforms [[buffer(binding)]];
//              RO buffers [[buffer(uniform_count + binding - ro_texture_count)]];
//              RW buffers [[buffer(uniform_count + ro_buffer_count + binding - rw_texture_count)]]

// MSL texture type behind an image2D's SPIR-V format enum.
static const char* cspv_msl_image_type(int format, bool readonly)
{
	bool is_uint = format == 32 || format == 33; // rgba8ui, r32ui
	if (readonly) return is_uint ? "texture2d<uint, access::read>" : "texture2d<float, access::read>";
	return is_uint ? "texture2d<uint, access::read_write>" : "texture2d<float, access::read_write>";
}

// Buffer-struct member spelling: MSL float3 is 16 bytes, so vec3 members use
// the packed_ forms to land on std430 offsets (pads handle alignment).
static const char* cspv_msl_member_type_name(cspv_type* t)
{
	if (t->kind == CSPV_T_VEC && t->cols == 3) {
		switch (t->elem->kind) {
		case CSPV_T_FLOAT: return "packed_float3";
		case CSPV_T_INT: return "packed_int3";
		case CSPV_T_UINT: return "packed_uint3";
		default: break;
		}
	}
	return cspv_hlsl_type_name(t);
}

// Natural MSL size of a member as declared by cspv_msl_member_type_name.
static void cspv_msl_natural_layout(cspv_type* t, int* align, int* size)
{
	if (t->kind == CSPV_T_VEC) {
		if (t->cols == 2) { *align = 8; *size = 8; return; }
		if (t->cols == 3) { *align = 4; *size = 12; return; } // packed_ form.
...
// Emit a struct whose members land on explicit target offsets (std140 for
// uniform blocks, std430 for buffer structs), padding with floats as needed.
static void cspv_msl_offset_struct(cspv_tp* g, const char* name, int num_members,
	cspv_type** member_types, const char** member_names, bool std140_offsets)
{
	cspv_ctx* ctx = g->ctx;
	sfmt_append(ctx->tp_out, "struct %s\n{\n", name);
	int natural = 0;
	...
	for (int i = 0; i < num_members; i++) {
		cspv_type* mt = member_types[i];
		if (mt->kind == CSPV_T_ARRAY && mt->cols == -1) continue; // Runtime tails are the pointer's element type.
		int align = 0, size = 0;
		if (std140_offsets) cspv_std140_layout(ctx, mt, 0, &align, &size);
		else cspv_std430_layout(ctx, mt, 0, &align, &size);
		offset = (offset + align - 1) / align * align;
		int nat_align = 0, nat_size = 0;
		cspv_msl_natural_layout(mt, &nat_align, &nat_size);
		while (natural < offset) {
...
static void cspv_msl_call(cspv_tp* g, cspv_expr* e)
{
	...
	// Type constructors: MSL is GLSL-shaped here (column-major matrix fill,
	// scalar splats, diagonal from a single scalar) with one exception --
	...
	if (ctor && e->u.call.array_size == -1) {
		...
		if (argc == 1 && ctor->kind == CSPV_T_VEC) {
			...
			if (a0 && a0->kind == CSPV_T_VEC && a0->cols > ctor->cols) {
				...
				spush(*out, '(');
				cspv_tp_expr(g, args[0], 15);
				sfmt_append(*out, ").%.*s", ctor->cols, swz[ctor->cols - 2]);
				return;
			}
		}
		sfmt_append(*out, "%s(", cspv_hlsl_type_name(ctor));
	...
	if (argc >= 1 && args[0]->kind == CSPV_E_REF && args[0]->rtype && args[0]->rtype->kind == CSPV_T_SAMPLER2D) {
		...
		if (sdim == CSPV_SDIM_2D_ARRAY_SHADOW && !strcmp(name, "texture")) {
			// depth2d_array comparison fetch: sample_compare(sampler, float2 uv,
			// uint layer, float ref). The layer splits out of P.z like the plain
			// array-texture path; the reference is P.w. Level zero for the same
			// reasons as the other shadow dims. (textureLod has no GLSL overload
			// for sampler2DArrayShadow -- the checker already rejected it.)
		...
		if (!strcmp(name, "textureOffset")) {
			if (arrayed) {
				// Layer splits out; the int2 offset stays the trailing argument.
				sfmt_append(*out, "%s_tex.sample(%s_smp, (", s, s);
				cspv_tp_expr(g, args[1], 2);
				sappend(*out, ").xy, uint(rint((");
				cspv_tp_expr(g, args[1], 2);
		...
		if (!strcmp(name, "texelFetch")) {
			if (arrayed) {
				// texture2d_array::read(uint2 coord, uint array, uint lod).
				sfmt_append(*out, "%s_tex.read(uint2((", s);
				cspv_tp_expr(g, args[1], 2);
				sappend(*out, ").xy), uint((");
				cspv_tp_expr(g, args[1], 2);
				sappend(*out, ").z), uint(");
			...
			if (sdim == CSPV_SDIM_3D) {
				sfmt_append(*out, "%s_tex.read(uint3(", s);
				cspv_tp_expr(g, args[1], 2);
				sappend(*out, "), uint(");
				cspv_tp_expr(g, args[2], 2);
				sappend(*out, "))");
		...
		if (!strcmp(name, "textureSize")) {
			if (arrayed || sdim == CSPV_SDIM_2D_ARRAY_SHADOW) {
				// ivec3(width, height, layers); get_array_size takes no lod.
				sfmt_append(*out, "int3(%s_tex.get_width(uint(", s);
				cspv_tp_expr(g, args[1], 2);
				sfmt_append(*out, ")), %s_tex.get_height(uint(", s);
				cspv_tp_expr(g, args[1], 2);
				sfmt_append(*out, ")), %s_tex.get_array_size())", s);
				return;
			}
			if (sdim == CSPV_SDIM_3D) {
				sfmt_append(*out, "int3(%s_tex.get_width(uint(", s);
				...
				sfmt_append(*out, ")), %s_tex.get_height(uint(", s);
				cspv_tp_expr(g, args[1], 2);
				sfmt_append(*out, ")), %s_tex.get_depth(uint(", s);
				cspv_tp_expr(g, args[1], 2);
				sappend(*out, ")))");
				return;
			}
			sfmt_append(*out, "int2(%s_tex.get_width(uint(", s);
			cspv_tp_expr(g, args[1], 2);
			sfmt_append(*out, ")), %s_tex.get_height(uint(", s);
			cspv_tp_expr(g, args[1], 2);
			sappend(*out, ")))");
	...
	if (!strcmp(name, "imageSize")) {
		...
		sappend(*out, "int2(");
		cspv_tp_expr(g, args[0], 15);
		sappend(*out, ".get_width(), ");
		cspv_tp_expr(g, args[0], 15);
		sappend(*out, ".get_height()))");
		return;
	}
	...
	if (!strcmp(name, "barrier") || !strcmp(name, "groupMemoryBarrier") || !strcmp(name, "memoryBarrierShared")) {
		sappend(*out, "threadgroup_barrier(mem_flags::mem_threadgroup)");
		return;
	}
	if (!strcmp(name, "memoryBarrier") || !strcmp(name, "memoryBarrierBuffer") || !strcmp(name, "memoryBarrierImage")) {
		sappend(*out, "threadgroup_barrier(mem_flags::mem_device)");
		return;
	}
	...
	// offsets to the component instead. Statement position only (validated).
	{
		...
		else if (!strcmp(name, "atomicMin")) aop = "atomic_fetch_min_explicit";
		else if (!strcmp(name, "atomicMax")) aop = "atomic_fetch_max_explicit";
		else if (!strcmp(name, "atomicAnd")) aop = "atomic_fetch_and_explicit";
		else if (!strcmp(name, "atomicOr")) aop = "atomic_fetch_or_explicit";
		...
		if (aop) {
			...
			while (root->kind == CSPV_E_INDEX) {
				...
				while (root->kind == CSPV_E_MEMBER) root = root->u.member.base;
			}
			bool threadgroup_space = false;
			if (root->kind == CSPV_E_REF) {
				for (int i = 0; i < (int)asize(g->ctx->decls); i++) {
					cspv_decl* d = g->ctx->decls + i;
	...
	// Bit casts: as_type sized by the argument's component count.
	{
		const char* target = NULL;
		if (!strcmp(name, "floatBitsToInt")) target = "int";
		else if (!strcmp(name, "floatBitsToUint")) target = "uint";
		else if (!strcmp(name, "intBitsToFloat") || !strcmp(name, "uintBitsToFloat")) target = "float";
		if (target) {
			cspv_type* at = cspv_tp_rtype(g, args[0]);
			int n = at ? cspv_num_components(at) : 1;
			if (n > 1) sfmt_append(*out, "as_type<%s%d>(", target, n);
			else sfmt_append(*out, "as_type<%s>(", target);
			cspv_tp_expr(g, args[0], 0);
			spush(*out, ')');
			return;
		}
	...
	// Pack/unpack: metal_pack functions plus as_type for the half pair.
	if (!strcmp(name, "packHalf2x16")) { sappend(*out, "as_type<uint>(half2("); cspv_tp_expr(g, args[0], 0); sappend(*out, "))"); return; }
	if (!strcmp(name, "unpackHalf2x16")) { sappend(*out, "float2(as_type<half2>("); cspv_tp_expr(g, args[0], 0); sappend(*out, "))"); return; }
	{
		const char* pk = NULL;
		if (!strcmp(name, "packUnorm4x8")) pk = "pack_float_to_unorm4x8";
		else if (!strcmp(name, "packSnorm4x8")) pk = "pack_float_to_snorm4x8";
		else if (!strcmp(name, "packUnorm2x16")) pk = "pack_float_to_unorm2x16";
		else if (!strcmp(name, "packSnorm2x16")) pk = "pack_float_to_snorm2x16";
		else if (!strcmp(name, "unpackUnorm4x8")) pk = "unpack_unorm4x8_to_float";
		else if (!strcmp(name, "unpackSnorm4x8")) pk = "unpack_snorm4x8_to_float";
		else if (!strcmp(name, "unpackUnorm2x16")) pk = "unpack_unorm2x16_to_float";
		else if (!strcmp(name, "unpackSnorm2x16")) pk = "unpack_snorm2x16_to_float";
		if (pk) {
			sfmt_append(*out, "%s(", pk);
			cspv_tp_expr(g, args[0], 0);
			spush(*out, ')');
			return;
		}
	...
	// Renames and the rest. Sampler-typed arguments expand into _tex/_smp pairs.
	const char* rn = NULL;
	if (!strcmp(name, "inversesqrt")) rn = "rsqrt";
	else if (!strcmp(name, "dFdx")) rn = "dfdx";
	else if (!strcmp(name, "dFdy")) rn = "dfdy";
	else if (!strcmp(name, "mod")) rn = "cf_mod";
	else if (!strcmp(name, "radians")) rn = "cf_radians";
	else if (!strcmp(name, "degrees")) rn = "cf_degrees";
	else if (!strcmp(name, "atan") && argc == 2) rn = "atan2";
...
// A user function as a cf_shader method. `in` array parameters pass as thread
// pointers (a documented aliasing divergence from GLSL's copy-in semantics;
// CF's shaders never mutate in-params they also read through the caller).
static void cspv_msl_func(cspv_tp* g, cspv_decl* d)
{
	cspv_ctx* ctx = g->ctx;
	bool is_main = d->name == g_cspv_kw.kw_main;
	sfmt_append(ctx->tp_out, "\t%s %s(", cspv_hlsl_type_name(d->type), is_main ? "cf_main_" : d->name);
	cspv_tp_push_shadows(g);
	bool first = true;
	for (int i = 0; i < d->num_params; i++) {
		if (!first) sappend(ctx->tp_out, ", ");
		...
		if (pt->kind == CSPV_T_SAMPLER2D) {
			...
			sfmt_append(ctx->tp_out, "thread %s& %s", cspv_hlsl_type_name(pt), pn); // out/inout.
		} else {
			sfmt_append(ctx->tp_out, "%s %s", cspv_hlsl_type_name(pt), pn);
		}
		apush(g->shadows, pn);
	}
...
static void cspv_emit_msl(cspv_ctx* ctx)
{
	// ---- Validate before printing (shares the HLSL restrictions). ----
	for (int i = 0; i < (int)asize(ctx->decls); i++) {
		cspv_decl* d = ctx->decls + i;
		if (d->kind == CSPV_D_BLOCK) {
			if (!d->is_buffer && d->instance_name) {
			...
			if (!d->is_buffer) {
				for (int j = 0; j < d->num_members; j++) {
					if (d->member_types[j]->kind == CSPV_T_MAT && d->member_types[j]->cols != 4) {
						cspv_errorf(ctx, d->line, "only mat4 is supported in uniform blocks for MSL output");
					}
				}
			}
		}
		if (d->kind == CSPV_D_FUNC) {
			for (int p = 0; p < d->num_params; p++) {
				if (d->params[p].type->kind == CSPV_T_IMAGE2D) {
					cspv_errorf(ctx, d->line, "image2D function parameters are not supported in MSL output");
				}
			}
			cspv_hlsl_validate_stmt(ctx, d->body);
		}
	...
	// ---- Resource index assignment (SDL_GPU's MSL contract; see header). ----
	// Counts per class first.
	int texture_count = 0;      // Graphics: sampled + storage textures. Compute: RO only.
	int rw_texture_count = 0;   // Compute set 1.
	int uniform_count = 0;
	int ro_buffer_count = 0;
	bool compute = ctx->stage == CSPV_STAGE_COMPUTE;
	for (int i = 0; i < (int)asize(ctx->decls); i++) {
		cspv_decl* d = ctx->decls + i;
		if (d->kind == CSPV_D_OPAQUE) {
			bool rw_image = d->type->kind == CSPV_T_IMAGE2D && !d->readonly;
			if (compute && rw_image) rw_texture_count++;
			else texture_count++;
		} else if (d->kind == CSPV_D_BLOCK) {
			if (!d->is_buffer) uniform_count++;
			else if (d->readonly) ro_buffer_count++;
		}
	}
	...
	// Uniform-block members resolve through the block's pointer member.
	for (int i = 0; i < (int)asize(ctx->decls); i++) {
		cspv_decl* d = ctx->decls + i;
		if (d->kind == CSPV_D_BLOCK && !d->is_buffer) {
			for (int j = 0; j < d->num_members; j++) {
				char* nm = (char*)cspv_arena_alloc(&ctx->arena, 128);
				snprintf(nm, 128, "%s->%s", d->name, d->member_names[j]);
				cspv_tp_rename(g, d->member_names[j], nm);
			}
		}
		// Scalar shared variables live behind a threadgroup pointer member.
		if (d->kind == CSPV_D_SHARED && d->type->kind != CSPV_T_ARRAY) {
			char* nm = (char*)cspv_arena_alloc(&ctx->arena, 96);
			snprintf(nm, 96, "(*%s)", d->name);
			cspv_tp_rename(g, d->name, nm);
		}
	}
	...
	// ---- Program-scope declarations: user structs, uniform-block structs,
	// const globals. Everything mutable lives inside cf_shader below. ----
	CK_DYNA cspv_type** buffer_structs = NULL;
	for (int i = 0; i < (int)asize(ctx->decls); i++) {
		cspv_decl* d = ctx->decls + i;
		if (d->kind != CSPV_D_BLOCK || !d->is_buffer) continue;
	...
	for (int i = 0; i < (int)asize(ctx->decls); i++) {
		...
		if (d->kind == CSPV_D_STRUCT) {
			...
			spush(ctx->tp_out, '\n');
			if (buffer_layout) {
				cspv_msl_offset_struct(g, d->name, (int)asize(d->type->field_types),
					d->type->field_types, d->type->field_names, false);
			} else {
				sfmt_append(ctx->tp_out, "struct %s\n{\n", d->name);
				for (int j = 0; j < (int)asize(d->type->field_types); j++) {
					spush(ctx->tp_out, '\t');
			...
			spush(ctx->tp_out, '\n');
			cspv_msl_offset_struct(g, sn, d->num_members, d->member_types, d->member_names, true);
		} else if (d->kind == CSPV_D_GLOBAL && d->is_const) {
			sappend(ctx->tp_out, "constant ");
			cspv_tp_var(g, d->type, d->name);
			if (d->init) {
				sappend(ctx->tp_out, " = ");
				cspv_tp_init(g, d->init);
	...
	// ---- The shader struct: resources first (aggregate-init order), then
	// mutable state, then every user function as a method. ----
	sappend(ctx->tp_out, "\nstruct cf_shader\n{\n");
	for (int i = 0; i < (int)asize(ctx->decls); i++) {
		cspv_decl* d = ctx->decls + i;
		if (d->kind == CSPV_D_BLOCK) {
		...
		} else if (d->kind == CSPV_D_SHARED) {
			cspv_type* st = d->type->kind == CSPV_T_ARRAY ? d->type->elem : d->type;
			sfmt_append(ctx->tp_out, "\tthreadgroup %s* %s;\n", cspv_hlsl_type_name(st), d->name);
		}
	}
	// Stage builtins and IO as members.
	if (ctx->stage == CSPV_STAGE_VERTEX) {
	...
	// Methods (complete-class context: definition order never matters).
	for (int i = 0; i < (int)asize(ctx->decls); i++) {
		cspv_decl* d = ctx->decls + i;
	...
	// ---- Stage IO structs and the main0 entry point. ----
	if (ctx->stage == CSPV_STAGE_VERTEX) {
		sappend(ctx->tp_out, "\nstruct cf_vs_in\n{\n");
		for (int i = 0; i < (int)asize(ctx->decls); i++) {
			cspv_decl* d = ctx->decls + i;
			if (d->kind != CSPV_D_INOUT || !d->is_input) continue;
			sfmt_append(ctx->tp_out, "\t%s %s [[attribute(%d)]];\n", cspv_hlsl_type_name(d->type), d->name, d->location);
		}
		sappend(ctx->tp_out, "};\nstruct cf_vs_out\n{\n\tfloat4 cf_position [[position]];\n");
		for (int i = 0; i < (int)asize(ctx->decls); i++) {
			cspv_decl* d = ctx->decls + i;
			if (d->kind != CSPV_D_INOUT || d->is_input) continue;
			sfmt_append(ctx->tp_out, "\t%s %s [[user(locn%d)]];\n", cspv_hlsl_type_name(d->type), d->name, d->location);
		}
		sappend(ctx->tp_out, "};\n");
	} else if (ctx->stage == CSPV_STAGE_FRAGMENT) {
		sappend(ctx->tp_out, "\nstruct cf_ps_in\n{\n\tfloat4 cf_position [[position]];\n");
		...
		for (int i = 0; i < (int)asize(ctx->decls); i++) {
			cspv_decl* d = ctx->decls + i;
			if (d->kind != CSPV_D_INOUT || d->is_input) continue;
			sfmt_append(ctx->tp_out, "\t%s %s [[color(%d)]];\n", cspv_hlsl_type_name(d->type), d->name, d->location);
		}
		sappend(ctx->tp_out, "};\n");
	}

	// Entry signature: stage IO plus every resource with its MSL index.
	if (ctx->stage == CSPV_STAGE_VERTEX) {
		sappend(ctx->tp_out, "\nvertex cf_vs_out main0(cf_vs_in cf_in [[stage_in]], uint cf_vid [[vertex_id]], uint cf_iid [[instance_id]]");
	} else if (ctx->stage == CSPV_STAGE_FRAGMENT) {
		sappend(ctx->tp_out, "\nfragment cf_ps_out main0(cf_ps_in cf_in [[stage_in]]");
	} else {
		sappend(ctx->tp_out,
			"\nkernel void main0(uint3 cf_gtid [[thread_position_in_threadgroup]], uint3 cf_dtid [[thread_position_in_grid]], "
			"uint3 cf_gid [[threadgroup_position_in_grid]], uint cf_gidx [[thread_index_in_threadgroup]]");
	}
	for (int i = 0; i < (int)asize(ctx->decls); i++) {
		cspv_decl* d = ctx->decls + i;
		if (d->kind == CSPV_D_BLOCK) {
	...
	// Threadgroup storage for shared variables, then aggregate-init the shader
	// struct (resources in member order; everything else value-initialized).
	for (int i = 0; i < (int)asize(ctx->decls); i++) {
		cspv_decl* d = ctx->decls + i;
		if (d->kind != CSPV_D_SHARED) continue;
		if (d->type->kind == CSPV_T_ARRAY) {
			sfmt_append(ctx->tp_out, "\tthreadgroup %s cf_sh_%s[%d];\n", cspv_hlsl_type_name(d->type->elem), d->name, d->type->cols);
		} else {
			sfmt_append(ctx->tp_out, "\tthreadgroup %s cf_sh_%s;\n", cspv_hlsl_type_name(d->type), d->name);
		}
	}
	sappend(ctx->tp_out, "\tcf_shader s = {");
	{
		bool first = true;
		for (int i = 0; i < (int)asize(ctx->decls); i++) {
			cspv_decl* d = ctx->decls + i;
			...
			if (d->kind == CSPV_D_BLOCK) {
				...
				if (d->type->kind == CSPV_T_SAMPLER2D) sfmt_append(ctx->tp_out, "%s%s_tex, %s_smp", sep, d->name, d->name);
				else sfmt_append(ctx->tp_out, "%s%s", sep, d->name);
				first = false;
			} else if (d->kind == CSPV_D_SHARED) {
				if (d->type->kind == CSPV_T_ARRAY) sfmt_append(ctx->tp_out, "%scf_sh_%s", sep, d->name);
				else sfmt_append(ctx->tp_out, "%s&cf_sh_%s", sep, d->name);
				first = false;
			}
		}
	}
	...
	// Bridge stage inputs and initialize mutable globals.
	if (ctx->stage == CSPV_STAGE_VERTEX) {
		for (int i = 0; i < (int)asize(ctx->decls); i++) {
			cspv_decl* d = ctx->decls + i;
			if (d->kind == CSPV_D_INOUT && d->is_input) sfmt_append(ctx->tp_out, "\ts.%s = cf_in.%s;\n", d->name, d->name);
		}
		sappend(ctx->tp_out, "\ts.gl_VertexIndex = int(cf_vid);\n\ts.gl_InstanceIndex = int(cf_iid);\n");
	} else if (ctx->stage == CSPV_STAGE_FRAGMENT) {
		sappend(ctx->tp_out, "\ts.gl_FragCoord = cf_in.cf_position;\n");
		for (int i = 0; i < (int)asize(ctx->decls); i++) {
			cspv_decl* d = ctx->decls + i;
			if (d->kind == CSPV_D_INOUT && d->is_input) sfmt_append(ctx->tp_out, "\ts.%s = cf_in.%s;\n", d->name, d->name);
		}
		...
		sappend(ctx->tp_out,
			"\ts.gl_GlobalInvocationID = cf_dtid;\n\ts.gl_LocalInvocationID = cf_gtid;\n"
			"\ts.gl_WorkGroupID = cf_gid;\n\ts.gl_LocalInvocationIndex = cf_gidx;\n");
	}
	for (int i = 0; i < (int)asize(ctx->decls); i++) {
		cspv_decl* d = ctx->decls + i;
		if (d->kind == CSPV_D_GLOBAL && !d->is_const && d->init) {
			sfmt_append(ctx->tp_out, "\ts.%s = ", d->name);
			cspv_tp_init(g, d->init);
			sappend(ctx->tp_out, ";\n");
		}
	}

	sappend(ctx->tp_out, "\ts.cf_main_();\n");

	if (ctx->stage == CSPV_STAGE_VERTEX) {
		sappend(ctx->tp_out, "\tcf_vs_out cf_out;\n\tcf_out.cf_position = s.gl_Position;\n");
		for (int i = 0; i < (int)asize(ctx->decls); i++) {
			cspv_decl* d = ctx->decls + i;
...
static void cspv_init_types(cspv_ctx* ctx)
{
	...
	ctx->t_sampler2darrayshadow = cspv_make_type(ctx, CSPV_T_SAMPLER2D, ctx->t_float, CSPV_SDIM_2D_ARRAY_SHADOW, 0);
	...
	cspv_register_type(ctx, "sampler2DArrayShadow", ctx->t_sampler2darrayshadow);
...
static void cspv_cleanup(cspv_ctx* ctx)
{
	...
	map_free(ctx->sampler_image_tids);
...
CSPV_Result cspv_compile_ex(const char* source, CSPV_Stage stage, const CSPV_Options* opts)
{
	...
	result.success = true;
	if (stage == CSPV_STAGE_COMPUTE) {
		ctx->reflection.local_size[0] = ctx->local_size[0];
		ctx->reflection.local_size[1] = ctx->local_size[1];
		ctx->reflection.local_size[2] = ctx->local_size[2];
	}
