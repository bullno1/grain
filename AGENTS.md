...
- `demos/` — standalone sample programs (plain CF apps, no bgame), one directory each, registered with `grain_add_demo(<name>)` in `demos/CMakeLists.txt` (target `grain-demo-<name>`). Each `<effect>.json` in a demo directory is baked by grainc into `grain_<effect>.h`; the effect files reference their modules by `path` (stock ones from `modules/`, demo-specific ones from `demos/<name>/modules/`), so the `.glsl` files stay the source of truth and grainc's depfile re-bakes on edit. `main.c` includes each header plainly at the top and again under `GRAIN_EFFECT_IMPLEMENTATION` at the bottom; tuned values come from the effect file via `grain_blueprint_apply`, and only per-frame values go through the typed `grain_set` handles. Desktop only (excluded from the web build).
