...
BTEST(archetype, sdf_builtins) {
	// The sdf library rides along grain/api.glsl: sd_*/op_* everywhere,
	// sdf_quad in the render vertex stage, sdf_mask in fragment stages
	grain_affector_t* affector = grain_define_affector(
		test_grain(),
		"Affector(SdfDeflect)\n"
		"Requires(\n"
		...
		")\n"
		"void process(inout ParticleAttrs particle, ModuleParams params, Ctx ctx) {\n"
		"	if (sd_circle(particle.position, params.radius) < 0.0) {\n"
	...
	grain_renderer_t* renderer = grain_define_renderer(
		...
		"#if GRAIN_SHADER_STAGE == GRAIN_SHADER_STAGE_VERTEX\n"
		"void process(ParticleAttrs particle, ModuleParams params, Ctx ctx) {\n"
		"	v_p = sdf_quad(vec2(params.radius + params.softness));\n"
		"	gl_Position = grain_transform * vec4(particle.position + v_p, 0.0, 1.0);\n"
		"}\n"
		"#elif GRAIN_SHADER_STAGE == GRAIN_SHADER_STAGE_FRAGMENT\n"
		"void process(ParticleAttrs particle, ModuleParams params, Ctx ctx) {\n"
		"	float d = op_smooth_union(\n"
		"		sd_circle(tf_scale(v_p, 2.0), params.radius) * 2.0,\n"
		"		sd_box(tf_rotate(tf_translate(v_p, vec2(1.0, 2.0)), 0.5), vec2(params.radius * 0.5)),\n"
		"		2.0\n"
		"	);\n"
		"	d = op_subtract(d, sd_segment(v_p, vec2(0.0), vec2(params.radius), 1.0));\n"
		"	vec4 c = vec4(1.0);\n"
		"	c.a *= sdf_mask(d) * sdf_mask(op_shell(d, 1.0), params.softness);\n"
		"	c.rgb *= sdf_glow(d, 2.0);\n"
...
BTEST(archetype, texture_smooth_builtin) {
	...
	grain_renderer_t* renderer = grain_define_renderer(
		...
		"#elif GRAIN_SHADER_STAGE == GRAIN_SHADER_STAGE_FRAGMENT\n"
		"void process(ParticleAttrs particle, ModuleParams params, Ctx ctx) {\n"
		"	vec2 uv = smooth_uv(atlas_uv(image_uvrect, v_uv), vec2(textureSize(image, 0)));\n"
		"	grain_Color = texture_smooth(image, image_uvrect, v_uv)\n"
		"		+ texture(image, uv) * 0.0;\n"
		"}\n"
		"#endif\n"
	);
	BTEST_ASSERT_EX(renderer != NULL, "%s", grain_get_last_error(test_grain()));
